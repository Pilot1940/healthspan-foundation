...
1. **Finding F5 (governance) — Dea(14) `is_minor=false` → RESOLVED 2026-06-15 (PC confirmed permanent).** **mig 072** added `profiles.is_minor_override` (bool) + `is_minor_override_note` (provenance) and set them for Dea (father/PC consent, permanent). A guard test (`tests/integration/test_rls_view_isolation.py::test_minor_framing_requires_explicit_override`) forbids ANY `relationship='child'` profile being adult-framed (active identity `is_minor=false`) without the override — so a future minor can't be silently adult-framed, and Dea's case is machine-evident. Docs/context reconciled (SYSTEM.md/.html, SCHEMA-MAP, context/dea.context.md). Behaviour unchanged (is_minor was already false everywhere). Operative flag is still `telegram_identities.is_minor`; the column declares the non-default value intentional.
2. **Finding F8 (data) — 8 `food_logs` rows with macro-sum > calories → 5 FIXED, 3 intentionally left (2026-06-15, full-DB audit pass).** Objective rule applied: only correct rows whose declared calories are *physically impossible* (below `protein×4 + carbs×4`); trust the stated macros, fix only the kcal; respect explicit user-typed kcal.
   - **Bucket B (clear error) — FIXED:** 2× 7-Eleven chicken breast (`d99a11dd`, `9a611944`) had `fat_g=5` vs the label's 1.5g in their own description → `fat 5→1.5g` (reconcile 90=90).
   - **Bucket A — 3 of 4 FIXED:** `alley` (`00bf447a` 180→244), `Morning whey` (`ba8d7398` 118→155), `WT coffee` (`5043fa54` 130→159) all had calories BELOW the protein+carb floor (impossible) → set calories = macro-derived, macros trusted, note appended. **`Spa`/Pre-onsen (`d570c8a1`) LEFT** — the user explicitly typed "280 kcal | …20g fat (full-fat milk base)" in the description, and 280 is *above* the floor; respect explicit input.
   - **Bucket C — LEFT (no impact):** 2× Dea `manual_import` "Daily nutrition total" (`7195bac8`, `77c138e3`) — `is_day_summary=true` (excluded from brief + per-meal analysis); the inconsistency is in her source tracker's export, not our extraction (rewriting it would falsify her imported history).
