...
| `sprint_set_adherence(sprint, date, activity, value, profile)` | DEFINER | Mig 066. Atomic, subtree-scoped write of `goals.adherence_log[date][activity]` (single `jsonb_set`). The TRACKING-surface write contract — Telegram `applyTick` + `lib/sprints.mark_done`. Ownership-gated (authenticated needs `has_profile_access`; service_role trusted, sprint pinned to `profile`). Returns the day's adherence map. |
