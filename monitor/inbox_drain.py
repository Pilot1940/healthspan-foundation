...
from lib import models  # central model-id registry (no hardcoded ids that can silently 404)
...
_VISION_PROMPTS: dict[str, str] = {
    "unknown": (
        ...
        'DATE/TIME: the timestamp shown as the default value below ("NOW") is the CURRENT '
        'date-time (UTC). When the user gives an explicit, partial, or relative date/time '
        '("10th June 8AM", "yesterday", "last night", "this morning"), resolve it against that '
        'current date-time and use the CURRENT YEAR unless the user explicitly names a different '
        'one — NEVER default to a past or future year. If the user gives no date/time, keep the '
        'default ("NOW"). Output the resolved value as an ISO-8601 timestamp.\n\n'
