...
4. **Preserve qualifiers** — `<10`, `>1000` keep their `qualifier` (the value is the bound). Never silently store the bound as an exact value.
   - **Abnormal ≠ staged.** A confidently-read value outside its reference range (high insulin, high android fat, low vitamin D) is REAL data — it writes to **prod** and is what this app exists to track. The contract returns `abnormal: True` + `range_flags`; CONFIRM it to the user ("ALT 78 is above the 0–55 range") but never route it to staging. `abnormal_labs` derives LOW/HIGH at query time. Only low *extraction confidence* / unresolved metric → staging.
